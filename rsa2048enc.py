...
tasklen=0
progress=0
def rsa_encrypt_task(offset,data,_p):
    return (offset,rsa.encrypt(data, _p))
...
def rsa_decrypt_task(offset,data,_pri):
    return (offset,rsa.decrypt(data, _pri))
...
def rsa_encrypt_multithreaded(biz_content, public_key, filename):
    ...
    while len_content - offset > 0:
        if len_content - offset > default_encrypt_length:
            pool.apply_async(func=rsa_encrypt_task, args=(offset,biz_content[offset: offset+default_encrypt_length],_p), callback=rsa_encrypt_task_callback)
        else:
            pool.apply_async(func=rsa_encrypt_task, args=(offset,biz_content[offset:],_p), callback=rsa_encrypt_task_callback)
    ...
    sorteddata=sorted(encrypteddata, key=lambda data: data[0])
    
    w=open(filename,'wb')
    for x,y in sorteddata:
        w.write(y)
        print("Merging... %.2d%% (%d/%d)"%(x/len_content*100,x,len_content))
    w.close()
...
def rsa_decrypt_multithreaded(biz_content, private_key, filename):
    ...
    while len_content - offset > 0:
        if len_content - offset > default_length:
            pool.apply_async(func=rsa_decrypt_task, args=(offset,biz_content[offset: offset+default_length],_pri), callback=rsa_decrypt_task_callback)
        else:
            pool.apply_async(func=rsa_decrypt_task, args=(offset,biz_content[offset:],_pri), callback=rsa_decrypt_task_callback)
    ...
    print("Data decrypted! Sorting...")
    sorteddata=sorted(decrypteddata, key=lambda data: data[0])
